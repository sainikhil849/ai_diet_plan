...
class MLAnalysisService:
    ...
    def _extract_features_from_data(self, numeric_data: Dict) -> Optional[np.ndarray]:
        """Extract features from numeric data in model format"""
        try:
            # Map extracted data to model features
            feature_mapping = {
                'Pregnancies': 0,  # Default for non-pregnant or male patients
                'Glucose': numeric_data.get('blood_sugar', numeric_data.get('glucose', 100)),
                'BloodPressure': self._parse_blood_pressure(numeric_data.get('blood_pressure', '120/80'))[0],
                'SkinThickness': 20,  # Default value
                'Insulin': numeric_data.get('insulin', 80),
                'BMI': numeric_data.get('bmi', 25),
                'DiabetesPedigreeFunction': 0.5,  # Default value
                'Age': numeric_data.get('age', 40)
            }
            
            # Create feature vector
            features = [feature_mapping.get(name, 0) for name in self.feature_names]
    ...
    def _rule_based_analysis(self, numeric_data: Dict) -> Dict:
        ...
        detected_conditions.extend(additional['conditions'])
